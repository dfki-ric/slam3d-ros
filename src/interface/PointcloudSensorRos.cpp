...
PointcloudSensorRos::PointcloudSensorRos(const std::string& sensor, const std::string robot)
{
	...
	ros::NodeHandle n((boost::format("/%1%") % sensor).str());
	...
	mPclSensor->setCovarianceScale(n.param("cov_scale", 1.0));
	
	n.param("scan_resolution", mScanResolution, 0.5);
	...
	n.param("use_seq_scan_matching", mSeqScanMatch, true);

	mPclSensor->setNeighborRadius(radius, links);
	mPclSensor->setMinPoseDistance(translation, rotation);
